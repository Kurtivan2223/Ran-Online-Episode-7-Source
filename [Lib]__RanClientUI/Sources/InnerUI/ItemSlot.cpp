		...
		for ( int i = 0; i < m_nMaxColumn; i++ )
		{
			...
			{	
				...
				if ( pItemData )
				{
					...
					const WORD wAttackNum = m_InvenItem[i].sItemCustom.PUBLICGETGRADEDMG();
					const WORD wDefenseNum = m_InvenItem[i].sItemCustom.PUBLICGETGRADEDEF();
...
void CItemSlot::SetInventoryNumber(int nIndex, int nNumber)
{
	CString strNumber;
	strNumber.Format("+%d", nNumber);
